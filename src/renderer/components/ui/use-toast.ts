...
function toast({ ...props }: Toast) {
  ...
  let duration = props.duration || DEFAULT_DURATION
  
  // If there's a description, calculate duration based on its length
  if (props.description) {
    ...
    // Minimum 3 seconds, maximum 10 seconds
    duration = Math.min(Math.max(contentLength * 40, DEFAULT_DURATION), 10000)
  }
